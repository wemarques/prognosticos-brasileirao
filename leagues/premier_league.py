...
class PremierLeague(BaseLeague):
    ...
    def get_dixon_coles_params(self) -> dict:
        ...
        Based on 2022-2024 season data.
        """
        return {
            'hfa': 1.40,  # Home field advantage
            'ava': 0.88,  # Away venue adjustment
            'league_avg_goals': 2.82,  # Average goals per game
            'rho': -0.15  # Correlation between home and away goals
        }
    ...
    def get_api_league_id(self, provider: str) -> str:
        ...
        """
        ids = {
            'football-data': 'PL',  # Código oficial Premier League
            'footystats': '2',     # ID do FootyStats
            'odds-api': 'soccer_epl'  # The Odds API (EPL = English Premier League)
        }
